...
`@layer` の順序保持を確認するには [tests/layer-order-fixture.html](tests/layer-order-fixture.html) を Chrome で直接開いてください。通常のレイヤー順、複数スタイルシートに分かれた `!important`、`@layer` を含めない設定を1ページで確認でき、抽出 CSS を貼り付けたプレビューの文字色も自動判定します。
...
- 元 CSS にある `@media`、`@supports`、`@container`、`@layer`、一部の疑似クラス・疑似要素、CSS カスタムプロパティ、`!important` を保持します。名前付きレイヤーは複数シートや通常の `@import` を含む初出順から順序宣言を再構成します。匿名レイヤーなど安全に順序を再構成できない場合は警告します。メディアクエリは現在の表示幅に関係なく収集します。
- 出力する宣言が `var()` で参照するカスタムプロパティは、選択要素で有効な定義を取得し、出力 CSS の先頭へ変数だけのブロックとしてまとめます。変数から別の変数を参照する場合も再帰的に収集します。取得できない変数は名前とフォールバックの有無を警告します。手入力モードや循環・無効な定義は自動取得できない場合があります。
...
- 完全な CSS Cascade は再現しません。同一文脈内では `!important`、単純セレクタの詳細度、元の出現順で競合を解決します。名前付きレイヤーの順序は保持しますが、`:where()`、継承、インラインスタイル、アニメーションは評価しません。抽出結果を必ず確認してください。
