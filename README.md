...
- 完全な CSS Cascade は再現しません。同一文脈内では `!important`、単純セレクタの詳細度、元の出現順で競合を解決します。名前付きレイヤーの順序と対応可能な関数内状態は保持しますが、複雑な関数型セレクタ、継承、インラインスタイル、アニメーションは評価しません。抽出結果を必ず確認してください。
