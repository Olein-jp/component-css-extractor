...
この文書は調査時点の記録である。レイヤー順序の欠落は Issue #7 で修正し、名前付きレイヤーの初出順から順序宣言を再構成するようになった。関数内状態の欠落は Issue #8 で修正し、`:where()` と `:is()` の単純な引数内にある状態を対象判定時だけ除くようになった。選択範囲外から継承した色は Issue #9 で診断を追加した。
