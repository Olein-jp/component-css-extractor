...
この文書は調査時点の記録である。レイヤー順序の欠落は Issue #7 で修正し、名前付きレイヤーの初出順から順序宣言を再構成するようになった。関数内状態の欠落は Issue #8 で修正し、`:where()` と `:is()` の単純な引数内にある状態を対象判定時だけ除くようになった。選択範囲外から継承した色は Issue #9、インラインスタイルの CSS 単独コピーに関する不足は Issue #10 で診断を追加した。
...
- `ElementNode.attributes` と `originalHtml` には `style` 属性が入り、生成 HTML に保持される。CSS 宣言の収集対象にはしないため、Issue #10 では選択範囲内に実質的な `style` 属性がある場合に CSS 単独コピーの不足を警告する。
- 選択範囲外の親要素の通常プロパティは、選択ノードと一致するルールとしては収集されない。Issue #9 では選択ルートの計算済み `color` が親要素と一致し、選択ルート自身に有効な色指定がない場合に継承依存を警告する。値を CSS 出力へ追加する処理は行わない。その他の通常プロパティは対象外。
...
3. **選択範囲外の継承**: Issue #9 で選択ルートの `color` に限定した診断を追加した。元 CSS を計算済み値で置き換えず、警告だけを表示する。
4. **インラインスタイル**: Issue #10 で CSS 単独利用時の不足を警告するようにした。既存の HTML と CSS の両方をコピーする動作は維持し、CSS 出力や優先順位計算は変更しない。
